...
/* ────────────────────────────────────────────────────────────────────────────
   ...
   client only) mounts the wallet tree, reads the chain and the monitor, and hands this file a
   filled-in `PortfolioView`. Keeping wagmi and RainbowKit out of here keeps them out of the server
   bundle.

   SHAPE. It follows theindex.finance's portfolio page, which readers of this chain already know: the
   connect bar, a band of metric cards, then the payment history beside a stack of smaller cards
   (holdings, the vaults, the line). The page is always the connected wallet's. It can also show
   another wallet, `?address=0x…`, but that is deliberately not offered anywhere on the page: no
   lookup field, no link to it. The only trace is `ViewingNote`, which says whose figures are on
   screen when they are not the connected wallet's, because a page headed "Your portfolio" must not
   print someone else's numbers without saying so.
   ──────────────────────────────────────────────────────────────────────────── */
...
/**
 * Every payment to the wallet, newest first, paged like the airdrops table. Turning a page also
 * brings the table back into view if its top has scrolled off, since the buttons sit under twelve
 * rows and the new rows would otherwise start a screen above the cursor.
 */
export function HistoryCard({ h }: { h: PortfolioView["history"] }) {
  const [page, setPage] = useState(0);
  const ref = useRef<HTMLDivElement>(null);
  const pageCount = Math.max(1, Math.ceil(h.rows.length / HISTORY_PAGE_SIZE));
  const safePage = Math.min(page, pageCount - 1);
  const visible = h.rows.slice(safePage * HISTORY_PAGE_SIZE, (safePage + 1) * HISTORY_PAGE_SIZE);
      ...
      <Card label="Airdrop history" action={<span style={{ ...mono, fontSize: 12, color: "var(--text-faint)" }}>{h.status}</span>}>
        {h.rows.length > 0 ? (
          <>
            ...
            <Pager page={safePage} pageCount={pageCount} total={h.rows.length} pageSize={HISTORY_PAGE_SIZE} noun="payments" label="Airdrop history pages" onPage={goTo} />
          </>
        ) : (
        ...
        <div style={{ borderTop: hairline, marginTop: 14, paddingTop: 14, fontSize: 13, color: "var(--text-muted)" }}>
          Every transfer from the airdrop wallet to this address, on the tokens the airdrop pays in, read from the chain. Newest first. Each payment is
          valued at what its cycle paid the token out at, not at today&apos;s price.
        </div>
      </Card>
    </div>
  );
      ...
      <div style={{ borderTop: hairline }}>
        {v.rows.map((r, i) => (
          <KVRow
            key={r.key}
            label={r.title}
            border={i === v.rows.length - 1 ? "none" : "bottom"}
            value={
...
export const STATIC_VIEW: PortfolioView = {
  metrics: {
    balance: {
      ...
      note: "Read from the OURO token on Robinhood Chain for the connected wallet, and refreshed every fifteen seconds.",
    },
    share: {
      value: DASH,
    ...
    rate: {
      value: DASH,
      footnote: "At the rate of the last seven days",
