...
/**
 * One-series daily bar chart, inline SVG, no library. Warm-neutral marks anchored to the baseline
 * with a rounded top, the latest day in bronze, a recessive baseline, and a hover tooltip. Days with
...
export function Bars({
  ...
  color?: string;
  /** The last bar, and a hovered one. */
  accent?: string;
  height?: number;
  ariaLabel: string;
  style?: CSSProperties;
    ...
    <div style={{ position: "relative", ...style }}>
        ...
        {data.map((d, i) => {
          ...
          return (
            <g key={d.t} onMouseEnter={() => setHover(i)}>
              ...
              {hh > 0 && <rect x={x} y={padTop + plotH - hh} width={barW} height={hh} rx={Math.min(2, barW / 2)} fill={on || last ? accent : color} />}
              {(i === 0 || last || (data.length > 14 && i % 7 === 0 && i < data.length - 3 && i > 2)) && (
                <text x={x + barW / 2} y={height - 6} textAnchor={last ? "end" : i === 0 ? "start" : "middle"} style={{ fontFamily: "var(--font-mono)", fontSize: 10, fill: "var(--text-faint)" }}>
                  {label(d.t)}
                </text>
              )}
            </g>
      ...
      {!hasData && (
        <div
          style={{
            position: "absolute",
            inset: `${padTop}px 0 ${padBottom}px 0`,
            display: "flex",
            alignItems: "center",
