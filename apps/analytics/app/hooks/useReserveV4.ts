...
import { dexQuote } from "~/lib/dexQuote";
 ...
 *
 * ONE reader for the whole page, not one per component. The Ledger states the same position twice,
 * in the treasury's total and in the position's own card, and two independent reads would have them
 * disagree by whatever the price moved between them, which is exactly the kind of small contradiction
 * this page cannot afford. So the poll lives here at module scope and every hook subscribes to it.
 *
